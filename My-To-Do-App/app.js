...
const formElement = document.getElementsByClassName('form-add-todo')[0];
const listElement = document.getElementsByClassName('list')[0];
const addToDoInputElement = document.getElementsByClassName('input-todo')[0];

//class names for icon indicators
const PENDING_ICON = 'co fa-solid fa-circle complete-icon';
const DONE_ICON = 'fa-solid fa-circle-check';
...
function clearAllToDo() {
  const clearIcon = document.getElementsByClassName('clear-icon');
...
function createToDoElement(id, text, status) {
  return `<li class="item" id="${id}">
    ...
    status === 'done' ? 'crossed' : ''
            ...
            <i class="fa-solid fa-trash remove-icon" id="deleteIcon-${id}" onclick="removeToDo(${id})"></i>
          </li>`;
}
...
const inputElement = document.getElementsByClassName('search')[0];
...
//filter data
 let ourResults = parsedLocalStorage.filter(function(oneToDo){
  return oneToDo.text.toLowerCase().includes(whatWeSearch.toLowerCase())
 });
 ...
 ourResults.forEach(function(oneToDo){
  let newItem = createToDoElement(oneToDo.id, oneToDo.text, oneToDo.status)
